// globalThis.SQLiteWASMFactory=
(e=>{
  "object"==typeof exports&&"undefined"!=typeof module?module.exports=e():
    "function"==typeof define&&define.amd?define(e):
    ("object"==typeof globalThis&&globalThis||this||self).SQLiteWASMFactory=e()
})(function(){
  var exports={},module={},result;
  ...
  */
  ...
  var initSqlJs=(moduleConfig,resolveModule,reject)=>{
  ...
  **Warning**: A statement belonging to a database that has been closed cannot
  ...
  if (ENVIRONMENT_IS_WORKER) { // Check worker, not web, since window could be polyfilled
    scriptDirectory = self.location+'';
  } else if ('object'===typeof document&&document.currentScript) { // web
  ...
  */
  ...
  if (id) {
    ...
    if(0)if (runDependencyWatcher === null && typeof setInterval !== 'undefined') {
  ...
  function instantiateAsync() {
    return moduleConfig.wasmInit(info,receiveInstantiatedSource)
    if (!wasmBinary &&
  ...
  Module["asm"].___emscripten_environ_constructor||(Module["asm"] = asm);
  ...
  */
  ...
  // }
  var init=(pre,rs,rj)=>{
    var d=!1,cb,r=initSqlJs({
      wasmInit:(info,rc)=>{cb=n=>d?(rc(n),rs(r)):d=n,pre(info,cb)}
      // await fetch('/src/js/lib/assets.sqljs_lite.wasm')
      //   .then(r=>r.arrayBuffer()).then(b=>((info,cb)=>WebAssembly.instantiate(b,info).then(cb)))
    },rs,rj);
    cb(d||!0);
  }
  return{
    init
    ,async:wasmURL=>new Promise((r,j)=>init(
      (info,cb)=>(
        WebAssembly.instantiateStreaming?WebAssembly.instantiateStreaming(fetch(wasmURL||'../bin/sqlite.wasm'),info):
        fetch(wasmURL||'../bin/sqlite.wasm').then(r=>r.arrayBuffer()).then(b=>WebAssembly.instantiate(b,info))
      ).then(cb),r,j
    ))
    // ,async:wasmURL=>new Promise(r=>init(
    //   (info,cb)=>fetch(wasmURL||'/src/js/lib/assets.sqljs_lite.wasm')
    //   .then(r=>r.arrayBuffer()).then(b=>WebAssembly.instantiate(b,info)).then(cb)
    //   ,r,console.error
    // ))
    ,sync:wasmURL=>{
      var x=new XMLHttpRequest,r;
      x.open('GET',wasmURL||'../bin/sqlite.wasm',!1);
      x.responseType='arraybuffer';
      x.send(null);
      return init((o,c)=>c({
        instance:new WebAssembly.Instance(new WebAssembly.Module(x.response),o)
      }),n=>r=n,j=>{throw j}),r;
      // var m=new WebAssembly.Module(x.response),f
      //   ,i=init((info,cb)=>f=()=>cb({instance:new WebAssembly.Instance(m,info)}),r=>r,j=>{throw j});
      // return f(),i
    },default:init
    // ,sync:wasmURL=>init(
    //   (info,cb)=>{
    //     var x=new XMLHttpRequest;
    //     x.open('GET',wasmURL||'/src/js/lib/assets.sqljs_lite.wasm',!1);
    //     x.responseType='arraybuffer';
    //     x.send(null);
    //     return cb({instance:new WebAssembly.Instance(new WebAssembly.Module(x.response),info)});
    //     // fetch().then(r=>r.arrayBuffer()).then(b=>WebAssembly.instantiate(b,info).then(receiveInstance))
    //   },r=>r,j=>{throw j}
    // ),default:init
  }
  /*
  ...
  */
  ...
  // {
  //     initWASM:f=>/* f(WebAssembly.imports,callback)
  //       * await fetch('/src/js/lib/assets.sqljs_lite.wasm')
  //       * .then(r=>r.arrayBuffer()).then(b=>((info,cb)=>WebAssembly.instantiate(b,info).then(cb)))
  //     */f(info,receiveInstance),
  //     default:()=>fetch('/src/js/lib/assets.sqljs_lite.wasm')
  //       .then(r=>r.arrayBuffer()).then(b=>WebAssembly.instantiate(b,info)).then(receiveInstance),
  //     sync:()=>{
  //       var x=new XMLHttpRequest;
  //       x.open('GET','/src/js/lib/assets.sqljs_lite.wasm',!0);
  //       x.responseType='arraybuffer';
  //       x.send(null);
  //       return receiveInstance(WebAssembly.Instance(x.response));
  //       // fetch().then(r=>r.arrayBuffer()).then(b=>WebAssembly.instantiate(b,info).then(receiveInstance))
  //     }
  //     // export:()=>
  //   }